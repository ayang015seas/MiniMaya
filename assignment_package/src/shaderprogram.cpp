...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      ...
      unifCamPos(-1), unifBindMatrices(-1), unifJointTransformations(-1), vertexInfluences(-1), vertexWeights(-1),
      firstMatrix(-1), secondMatrix(-1),
      context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    unifBindMatrices   = context->glGetUniformLocation(prog, "u_Bind");
    unifJointTransformations  = context->glGetUniformLocation(prog, "u_Joint");

    vertexInfluences = context->glGetAttribLocation(prog, "vs_Influence");
    vertexWeights = context->glGetAttribLocation(prog, "vs_Weight");

    firstMatrix = context->glGetUniformLocation(prog, "vs_First");
    secondMatrix = context->glGetUniformLocation(prog, "vs_Second");
...
void ShaderProgram::setBindMatrix(glm::mat4 bm[], int num) {
    useMe();
...
void ShaderProgram::setOverallTransformation(glm::mat4 tm[], int num) {
    useMe();

    if(unifJointTransformations != -1)
    {
        context->glUniformMatrix4fv(unifJointTransformations,
                                    num, GL_FALSE,
                                    glm::value_ptr(tm[0]));
    }
}
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    if (vertexInfluences != -1 && d.bindInfluences()) {
        context->glEnableVertexAttribArray(vertexInfluences);
        context->glVertexAttribIPointer(vertexInfluences, 2, GL_INT, 0, nullptr);
    }
    context->printGLErrorLog();


    if (vertexWeights != -1 && d.bindWeights()) {
        context->glEnableVertexAttribArray(vertexWeights);
        context->glVertexAttribPointer(vertexWeights, 2, GL_FLOAT, false, 0, nullptr);
    }
    context->printGLErrorLog();


//    if (firstMatrix != -1 && d.bindFirst()) {
//        context->glEnableVertexAttribArray(firstMatrix);
//        context->glVertexAttribPointer(firstMatrix, 2, false, 0, nullptr);
//    }

//    if (secondMatrix != -1 && d.bindSecond()) {
//        context->glEnableVertexAttribArray(vertexWeights);
//        context->glVertexAttribPointer(vertexWeights, 2, GL_FLOAT, false, 0, nullptr);
//    }
    context->printGLErrorLog();
    ...
    if (vertexWeights != -1) context->glDisableVertexAttribArray(vertexWeights);
    if (vertexInfluences != -1) context->glDisableVertexAttribArray(vertexInfluences);
